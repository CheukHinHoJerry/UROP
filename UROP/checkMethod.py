...
Dx1 = np.eye(N * N + 1, k=1) - np.eye(N * N + 1, k=-1)
Dx1 = Dx1 / (2 * h)
# First and last row of Dx1 is zero for applying boundary condition
Dx1[0, :] = np.zeros(N * N + 1)
Dx1[N * N, :] = np.zeros(N * N + 1)

Dx2 = np.eye(N * N + 1, k=1) + np.eye(N * N + 1, k=-1) - 2 * np.eye(N * N + 1, k=0)
Dx2 = Dx2 / (2 * h * 2 * h)

# For applying boundary condition
Dx2[0, :] = np.hstack((-1, np.zeros(N * N)))
Dx2[N * N, :] = np.hstack((np.zeros(N * N), -1))
...
sol = fsolve(coarseFunc, np.ones(N * N + 1))
print("The solution for is", sol)
real_coarse_sol = np.array([sol[0]])

for i in range(1, N + 1):
    real_coarse_sol = np.hstack((real_coarse_sol, sol[i * N]))
