...
#         print((fineSol[1:] - fineSol[:1000]) / h)
#
...
# solving the system with some initial guess
sol = fsolve(coarseFunc, np.ones(N * N ** 3 + 1))
# print("The solution for is", sol)

coarse_left_deri = np.zeros(N-1)
coarse_right_deri = np.zeros(N-1)
for i in range(1, N):
    coarse_left_deri[i-1] = (sol[i*N**3]-sol[i*N**3-1])/h
    coarse_right_deri[i-1] = (sol[i*N**3+1]-sol[i*N**3])/h
